...
###for loop that greps the columns of the drugs that we need: Ciprofloxacin, Amoxiline, PipTaz, Cefuroxime, Ceftazidime
# List of parts to check for in column names
parts <- c("cipro", "amoxi", "pi", "cefu", "cefta", "genta")

# Loop through each column name
for (col in names(Kallonnen_MetaData)) {
  # Check if any part is present in the column name (ignoring case)
  if (any(grepl(paste(parts, collapse = "|"), col, ignore.case = TRUE))) {
    cat("Column '", col, "' contains one of the specified parts.\n", sep = "")
  }
}
